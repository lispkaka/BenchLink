<template>
  ...
  <div>
      ...
      <el-card class="chart-card" shadow="hover">
        ...
        <VChart v-if="failureTrend.length > 0" class="chart" :option="barChartOption" :autoresize="true" />
        <el-empty v-else description="暂无失败数据" :image-size="100" />
      </el-card>
...
const barChartOption = computed(() => ({
  ...
  xAxis: {
    type: 'category',
    data: failureTrend.value.map((item) => item.name),
    axisLabel: {
      rotate: 45,
      interval: 0
    }
  ...
  series: [
    {
      name: '执行次数',
      type: 'bar',
      data: failureTrend.value.map((item) => item.runs),
      itemStyle: {
        color: '#3B82F6'
      }
    },
    {
      name: '失败次数',
      type: 'bar',
      data: failureTrend.value.map((item) => item.failures),
      itemStyle: {
        color: '#EF4444'
      }
    }
